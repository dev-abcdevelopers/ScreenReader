...
import com.bliss.screenreader.databinding.FragmentPolicyDetailBinding
...
class PolicyDetailFragment : Fragment() {
    ...
    private lateinit var ViewBindingObj: FragmentPolicyDetailBinding
    ...
    override fun onCreateView(
        ...
        ViewBindingObj = FragmentPolicyDetailBinding.inflate(inflater, container, false)
        return ViewBindingObj.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        val ArgsRef = requireArguments()
        IsEmbedded = ArgsRef.getBoolean(ARG_EMBEDDED, false)
        ApplyChrome()
        ApplyColumns(TwoColumn = ArgsRef.getBoolean(ARG_TWO_COLUMN, false))
        ...
        val PolicyNumber = ArgsRef.getString(ARG_POLICY_NUMBER).orEmpty()
        SessionIdVal = ArgsRef.getString(ARG_SESSION_ID).orEmpty()
    ...
    private fun ApplyChrome() {
        if (IsEmbedded) {
            ViewBindingObj.toolbar.navigationIcon = null
            return
        }
        ViewBindingObj.toolbar.setNavigationOnClickListener { CloseHost() }
        ViewCompat.setOnApplyWindowInsetsListener(ViewBindingObj.root) { _, WindowInsetsObj ->
            ...
            ViewBindingObj.toolbar.updatePadding(top = BarInsets.top)
            ViewBindingObj.root.updatePadding(bottom = BarInsets.bottom)
            WindowInsetsObj
        }
    }

    private fun ApplyColumns(TwoColumn: Boolean) {
        if (!TwoColumn) return
        ViewBindingObj.detailColumns.orientation = LinearLayout.HORIZONTAL
        val GapPx = resources.getDimensionPixelSize(R.dimen.detail_column_gap)
        ViewBindingObj.columnMain.layoutParams =
            LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f).apply {
                marginEnd = GapPx
            }
        ViewBindingObj.columnSide.layoutParams =
            LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f)
        val HeaderParams = ViewBindingObj.headerCustomer.layoutParams as LinearLayout.LayoutParams
        HeaderParams.topMargin = resources.getDimensionPixelSize(R.dimen.space_md)
        ViewBindingObj.headerCustomer.layoutParams = HeaderParams
    }

    private fun CloseHost() {
        if (IsEmbedded) {
            val HostRef = parentFragment as? Host ?: return
            ViewBindingObj.root.post { HostRef.OnDetailClosed() }
        } else {
            activity?.finish()
        }
    }

    private fun AfterCaptureStarted() {
        if (!IsEmbedded) activity?.finish()
    }


    private fun RenderAdvancedVisibility() {
        ViewBindingObj.toolbar.menu.findItem(R.id.actionExportPolicy)?.isVisible =
            SettingsStore.IsSessionExportVisible(ContextRef = requireContext())
        RenewalRowBinding?.root?.visibility =
            if (SettingsStore.IsRenewalHistoryVisible(ContextRef = requireContext())) {
                View.VISIBLE
            } else {
                View.GONE
            }
    ...
    private fun BindStatusChip(PolicyRef: CustomerPolicy) {
        ...
        ViewBindingObj.chipStatus.chipBackgroundColor =
            android.content.res.ColorStateList.valueOf(
                ContextCompat.getColor(requireContext(), StatusBackgroundFor(StatusText = StatusText))
            )
        ViewBindingObj.chipStatus.setTextColor(
            ContextCompat.getColor(requireContext(), StatusTextFor(StatusText = StatusText))
        )
    }
    ...
    private fun AddChip(LabelText: String, BackgroundRes: Int, TextColorRes: Int) {
        val ChipRef = Chip(requireContext())
        ChipRef.text = LabelText
        ChipRef.isClickable = false
        ChipRef.isCheckable = false
        ChipRef.chipBackgroundColor =
            android.content.res.ColorStateList.valueOf(ContextCompat.getColor(requireContext(), BackgroundRes))
        ChipRef.setTextColor(ContextCompat.getColor(requireContext(), TextColorRes))
    ...
    private fun BindVerdict(PolicyRef: CustomerPolicy) {
        ...
        ViewBindingObj.tvVerdictLead.setTextColor(ContextCompat.getColor(requireContext(), LeadColourRes))
    ...
    private fun BindLife(PolicyRef: CustomerPolicy) {
        ...
        for ((StopIndex, StopItem) in StopList.withIndex()) {
            ...
            StopBinding.railDot.backgroundTintList =
                android.content.res.ColorStateList.valueOf(
                    ContextCompat.getColor(requireContext(), DotColourRes)
                )
            ...
            StopBinding.tvStopValue.setTextColor(ContextCompat.getColor(requireContext(), ValueColourRes))
            if (IsNow) {
                StopBinding.tvStopLabel.setTextColor(
                    ContextCompat.getColor(requireContext(), R.color.text_accent)
                )
            }
        ...
        RowBinding.rowRenewalHistory.setOnClickListener { ViewRef ->
            ...
            if (RenewalList.isEmpty()) {
                ...
                RenewalHistorySheet.NewInstance(PolicyNumber = PolicyRef.PolicyNumber)
                    .show(childFragmentManager, RenewalHistorySheet.TAG)
            }
        }
    }
    ...
    private fun ResumeCaptureForPolicy() {
        ...
        val StartedOk = CaptureFlow.Start(
            ActivityRef = requireActivity() as AppCompatActivity,
        ...
        if (StartedOk) AfterCaptureStarted()
    }
    ...
    private fun ResolveSessionId(): String {
        return SessionIdVal.ifEmpty {
            PolicyRepository.GetLatestSessionId(ContextRef = requireContext(), ModeVal = CaptureMode.POLICY)
        }
    }
    ...
    private fun CopyValue(LabelText: String, ValueText: String) {
        ...
        val ClipboardRef = requireContext().getSystemService(Context.CLIPBOARD_SERVICE) as? ClipboardManager ?: return
    ...
    private fun ShowMessage(
        ...
        AppToast.Show(ContextRef = requireContext(), MessageText = MessageVal, KindVal = KindVal)
    }
    ...
    private fun ExportSingle(PolicyRef: CustomerPolicy) {
        ...
        val FileUri = FileProvider.getUriForFile(
            ContextRef,
            "${ContextRef.packageName}.fileprovider",
            ExportedFile
        )
    ...
    private fun BuildLabels(): PolicyCompleteness.LabelSet {
        return CompletenessLabels.From(ContextRef = requireContext())
    }

    companion object {
        const val TAG = "policy_detail"
        ...
        private const val ARG_TWO_COLUMN = "arg_two_column"
